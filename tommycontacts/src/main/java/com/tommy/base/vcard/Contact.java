...
public class Contact {
		...
		handleProp orgHandler = new handleProp() {
			...
			public void parseProp(String propName, Vector<String> propVec, String val) {
				...
				if (propName.equals("TITLE")) {
					...
					if (!setTitle) {
						orgs.add(new OrgData(label == null ? Organization.TYPE_WORK : Organization.TYPE_CUSTOM, val, null, label));
					}
					...
					if (!setCompany) {
						orgs.add(new OrgData(label == null ? Organization.TYPE_WORK : Organization.TYPE_CUSTOM, null, orgFields[0], label));
					}
				}
			}
		};
	...
	final static Pattern[] phonePatterns = { Pattern.compile("[+](1)(\\d\\d\\d)(\\d\\d\\d)(\\d\\d\\d\\d.*)"), Pattern.compile("[+](972)(2|3|4|8|9|50|52|54|57|59|77)(\\d\\d\\d)(\\d\\d\\d\\d.*)"), };
	...
	public static void formatPeople(Appendable formatted, PeopleData peoData) throws IOException {
		formatted.append("N");
		if (!StringUtils.isASCII(peoData.midName) || !StringUtils.isASCII(peoData.familyNmae))
			formatted.append(";CHARSET=UTF-8");
		if (peoData.midName != null) {
			formatted.append(":").append((peoData.givenName != null) ? peoData.givenName.trim() : "").append(";").append((peoData.familyNmae != null) ? peoData.familyNmae.trim() : "").append(";")
					.append((peoData.midName != null) ? peoData.midName.trim() : "").append(";").append(";").append(NL);
		} else {
			formatted.append(":").append((peoData.familyNmae != null) ? peoData.familyNmae.trim() : "").append(";").append((peoData.givenName != null) ? peoData.givenName.trim() : "").append(";")
	...
	private void getOrgFields(ContentResolver cResolver) {
		...
		String[] projection = { Organization.TITLE, Organization.COMPANY, Organization.TYPE, Organization.LABEL };
	...
	private void getImFields(ContentResolver cResolver) {
		// 从Data中取出Im数据
		String customType = null;
		String[] projection = { Im.DATA, Im.TYPE, Im.IS_PRIMARY, Im.LABEL, Im.PROTOCOL, Im.CUSTOM_PROTOCOL };
	...
	private void getAddressFields(ContentResolver cResolver) {
		...
		String[] projection = { StructuredPostal.POBOX, StructuredPostal.STREET, StructuredPostal.CITY, StructuredPostal.REGION, StructuredPostal.POSTCODE, StructuredPostal.COUNTRY,
				StructuredPostal.IS_PRIMARY, StructuredPostal.TYPE, StructuredPostal.LABEL };
		StringBuffer dataBuff = new StringBuffer();
		String selection = Data.CONTACT_ID + " = ? AND " + Data.MIMETYPE + " = ?";
		String[] selParams = new String[] { peopleData._id, StructuredPostal.CONTENT_ITEM_TYPE };
		Cursor postalCur = cResolver.query(Data.CONTENT_URI, projection, selection, selParams, null);
	...
	private void getPeopleFields(ContentResolver cResolver) {
		...
		String[] projection = { StructuredName.DISPLAY_NAME, StructuredName.FAMILY_NAME, StructuredName.MIDDLE_NAME, StructuredName.GIVEN_NAME };
		String selection = Data.CONTACT_ID + " = ? AND " + Data.MIMETYPE + " = ?";
	...
	private void RemoveContact(ContentResolver cResolver, long key) {
		// 移除已有联系人
		if (key != 0) {
			cResolver.delete(RawContacts.CONTENT_URI, RawContacts._ID + " = " + key, null);
		} else {
	...
	public long addContact(Context context, long key, boolean replace) {
		...
		Cursor people = cResolver.query(Data.CONTENT_URI, projection, selection, selParams, null);
	...
	public ContentValues getOrgnizeCV(OrgData org) {
		...
		if (StringUtils.isNullOrEmpty(org.company) && StringUtils.isNullOrEmpty(org.title)) {
			return null;
		}
		ContentValues cv = new ContentValues();
		// 类型
