...
import com.tommy.base.vcard.VCardIO;
import com.tommy.bean.BackList;
import com.tommy.bean.ContactBean;
import com.tommy.bean.GroupBean;
import com.tommy.bean.KuaiXu;
import com.tommy.util.AbToastUtil;
import com.tommy.util.BaseIntentUtil;
import com.tommy.view.adapter.ContactHomeAdapter;
import com.tommy.view.adapter.MenuListAdapter;
import com.tommy.view.other.SizeCallBackForMenu;
import com.tommy.view.sms.MessageBoxList;
import com.tommy.view.ui.MenuHorizontalScrollView;
...
public class HomeContactActivity extends Activity {
	...
	private String[] lianxiren1 = new String[] { "发送短信", "查看详细", "删除", "加入黑名单", "拨打电话" };

	// 群组联系人弹出页
	private void showContactDialog(final String[] arg, final ContactBean cb, final int position) {
		new AlertDialog.Builder(this).setTitle(cb.getDisplayName()).setItems(arg, new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {
				...
				case 3:// 加入黑名单
					...
					break;
				case 4:
					Intent intent = new Intent(Intent.ACTION_CALL, Uri.parse("tel:" + cb.getPhoneNum()));
	...
	public void showJianPan(String number) {
		// 将布局文件转化成view对象
		LayoutInflater inflaterDl = LayoutInflater.from(this);
		LinearLayout layout = (LinearLayout) inflaterDl.inflate(R.layout.bohao_dialog, null);
		...
		Button dialNum0 = (Button) layout.findViewById(R.id.dialNum0);
		Button dialNum1 = (Button) layout.findViewById(R.id.dialNum1);
		Button dialNum2 = (Button) layout.findViewById(R.id.dialNum2);
		Button dialNum3 = (Button) layout.findViewById(R.id.dialNum3);
		Button dialNum4 = (Button) layout.findViewById(R.id.dialNum4);
		Button dialNum5 = (Button) layout.findViewById(R.id.dialNum5);
		Button dialNum6 = (Button) layout.findViewById(R.id.dialNum6);
		Button dialNum7 = (Button) layout.findViewById(R.id.dialNum7);
		Button dialNum8 = (Button) layout.findViewById(R.id.dialNum8);
		Button dialNum9 = (Button) layout.findViewById(R.id.dialNum9);
		dialNum0.setOnClickListener(new MyOnclick(0, number));
		dialNum1.setOnClickListener(new MyOnclick(1, number));
		dialNum2.setOnClickListener(new MyOnclick(2, number));
		dialNum3.setOnClickListener(new MyOnclick(3, number));
		dialNum4.setOnClickListener(new MyOnclick(4, number));
		dialNum5.setOnClickListener(new MyOnclick(5, number));
		dialNum6.setOnClickListener(new MyOnclick(6, number));
		dialNum7.setOnClickListener(new MyOnclick(7, number));
		dialNum8.setOnClickListener(new MyOnclick(8, number));
		dialNum9.setOnClickListener(new MyOnclick(9, number));
	...
	class MyOnclick implements OnClickListener {
		...
		MyOnclick(int number, String phone) {
			this.number = number;
			this.phone = phone;
		}
		...
		public void onClick(View v) {
			try {
				dialog.dismiss();
				KuaiXu kuaiXu = new KuaiXu();
				kuaiXu.setNumber(number);
				kuaiXu.setPhone(phone);
				dbUtils.save(kuaiXu);
				Toast.makeText(HomeContactActivity.this, "设置成功", Toast.LENGTH_SHORT).show();
			} catch (Exception e) {
	...
	private void showDelete(final int contactsID, final int position) {
		new AlertDialog.Builder(this).setIcon(R.drawable.ic_launcher).setTitle("是否删除此联系人").setPositiveButton("确定", new DialogInterface.OnClickListener() {
	...
	private void queryGroupMember(GroupBean gb) {
		...
		Cursor cur = getContentResolver().query(ContactsContract.Data.CONTENT_URI, RAW_PROJECTION,
				ContactsContract.Data.MIMETYPE + " = '" + GroupMembership.CONTENT_ITEM_TYPE + "' AND " + ContactsContract.Data.DATA1 + "=" + gb.getId(), null, "data1 asc");
		...
		Cursor contactIdCursor = getContentResolver().query(ContactsContract.RawContacts.CONTENT_URI, new String[] { ContactsContract.RawContacts.CONTACT_ID }, inSelectionBff.toString(), null,
