...
import com.tommy.bean.CallLogBean;
import com.tommy.bean.KuaiXu;
import com.tommy.view.adapter.HomeDialAdapter;
import com.tommy.view.adapter.T9Adapter;
import com.tommy.R;
...
public class HomeDialActivity extends Activity implements OnClickListener {
				...
				public void onTextChanged(CharSequence s, int start, int before, int count) {
					if (null == application.getContactBeanList() || application.getContactBeanList().size() < 1 || "".equals(s.toString())) {
						...
						if (null == t9Adapter) {
							...
							// TextView tv = new
							// TextView(HomeDialActivity.this);
	...
	@OnLongClick({ R.id.dialNum0, R.id.dialNum1, R.id.dialNum2, R.id.dialNum3, R.id.dialNum4, R.id.dialNum5, R.id.dialNum6, R.id.dialNum7, R.id.dialNum8, R.id.dialNum9 })
	public boolean onLongClick(View v) {
		// TODO Auto-generated method stub
		int number = 0;
		switch (v.getId()) {
	 ...
	 * 快速拨号
	 */
	public void bohao(int number) {
		DbUtils dbUtils = DbUtils.create(this);
		try {
			List<KuaiXu> kuaiXus = dbUtils.findAll(KuaiXu.class);
			if (kuaiXus != null) {
				for (int i = 0; i < kuaiXus.size(); i++) {
					if (kuaiXus.get(i).getNumber() == number) {
						Intent intent = new Intent(Intent.ACTION_CALL, Uri.parse("tel:" + kuaiXus.get(i).getPhone()));
